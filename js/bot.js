...
const BotPeer = (() => {
  ...
  //
  // GLOBAL SAFETY NET: every single path that can ever show the typing
  // bubble (banter, guess-ticks, conversational replies) funnels through
  // this one function to acquire the lock — so this is the single choke
  // point to guarantee the lock can NEVER stay held forever, no matter
  // what bug might exist (now or introduced later) in whatever code runs
  // between acquiring it and calling finishTypingAndSend. A live report
  // showed the bubble stuck indefinitely with no reply ever arriving;
  // rather than only patch the one specific path that caused it (see the
  // conversational-reply handler's own additional watchdog around
  // BotChat.getReply for that particular fix), this unconditionally
  // force-releases the lock 10s after every acquisition if nothing else
  // has released it by then — sending a short local fallback line so
  // the round never just goes silent. 10s is intentionally longer than
  // any legitimate single "compose" cycle should ever take (the longest
  // being the conversational-reply path's own 7s watchdog plus its
  // ~2.6s max typing-out delay), so this should essentially never fire
  ...
  function acquireSpeakerLock() {
    ...
    const myToken = {};
    pendingSpeakerToken = myToken;
    messageHandlers.forEach(fn => fn({ type: 'typing_start', name: botName }));
    setTimeout(() => {
      if (pendingSpeakerToken !== myToken) return; // already released normally — nothing to do
      messageHandlers.forEach(fn => fn({ type: 'typing_stop' }));
      pendingSpeakerToken = null;
      if (!roundOver) {
        emit({ type: 'chat', text: BotChat.localReply(''), name: botName, msgId: 'bot' + Date.now() });
      }
      runQueuedSpeakIfAny();
    }, 10000);
    return myToken;
  ...
  // finishTypingAndSend takes the token the caller was given by
  // acquireSpeakerLock (or omits it, meaning "I never tried to
  // pre-acquire, take the lock fresh right now"). If a token IS passed
  // but no longer matches the live pendingSpeakerToken, someone else has
  // since taken over the lock (e.g. this call is a stale promise
  // resolving late after a different message already finished and moved
  // on) — in that case this is a safe no-op rather than corrupting the
  // current, unrelated holder's in-flight bubble/timer.
  //
  // IMPORTANT: this function NEVER opens the lock itself anymore — it
  // always goes through acquireSpeakerLock() for that (see the
  // `!token` branch below), because acquireSpeakerLock is the ONE place
  // with the 10s force-release safety net. A live report showed the
  // typing bubble getting stuck forever with no reply ever arriving;
  // root cause was this function previously having its OWN separate,
  // hand-rolled lock-acquisition (raw `pendingSpeakerToken = {}` +
  // typing_start) for the "no token passed" case — the exact path
  // banter's emitChat() uses on every single firing — which had NO
  // watchdog protecting it. If its own completion setTimeout below ever
  // failed to fire for any reason, that lock had zero backstop and
  // stayed stuck permanently. Routing everything through the single
  // acquireSpeakerLock() closes that gap: now there is exactly one
  // place in the whole module that ever sets pendingSpeakerToken to a
  // fresh value, and it always carries its own independent watchdog.
  function finishTypingAndSend(text, token) {
    ...
    let myToken = token;
    if (!myToken) {
      myToken = acquireSpeakerLock(); // now the ONLY place that opens a fresh lock — always carries the 10s safety net
      if (!myToken) return; // lock is held by someone else (shouldn't normally happen for an untokened call, but bail safely rather than fight over it) or round just ended
    }
    ...
    setTimeout(() => {
      ...
      // provably safe to reason about on its own. This same check is
      // also what makes it safe that acquireSpeakerLock's OWN 10s
      // safety-net timer is still separately running alongside this
      // one for the same token — this (normal, ~0.5-2.6s) timer will
      // always fire first and clear pendingSpeakerToken, so when the
      // safety net's timer eventually fires 10s later it sees the
      // token no longer matches and correctly no-ops instead of
      // double-sending.
